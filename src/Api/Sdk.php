...
namespace Streamchat\Api;

if (!class_exists('Streamchat\Api\Exception\Exception')) {
...
use LogicException;
use Streamchat\Api\Exception\AuthException;
use Streamchat\Api\Exception\AuthRequiredException;
use Streamchat\Api\Exception\Exception;
use Streamchat\Api\Exception\InvalidInputException;
...
class Sdk
{
     ...
     * @param string $host
     */
    public function __construct($host = 'https://api.streamchat.fr/', $token = null)
    {
        $this->host = $host;
    ...
    public function authenticate($login, $password)
    {
        ...
        try {
            ...
            $result = $this->call('authent/token', 'POST', [
                'login' => $login,
                'password' => $password,
            ]);
     ...
     * @throws Exception
     */
    private function prepareRequest(string $path, $verb, array $args, array $files)
    {
